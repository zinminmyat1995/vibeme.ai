...
const PRESET_DEDUCTIONS = [
    { name: 'Income Tax',        type: 'percentage', emoji: '🏛️' },
    { name: 'Social Security',   type: 'percentage', emoji: '🛡️' },
    { name: 'NSSF',              type: 'percentage', emoji: '📋' },
    { name: 'SSB',               type: 'percentage', emoji: '🏦' },
    { name: 'Health Insurance',  type: 'percentage', emoji: '🏥' },
    { name: 'Pension Fund',      type: 'percentage', emoji: '💰' },
    { name: 'Union Fee',         type: 'flat',       emoji: '🤝' },
    { name: 'Absence Deduction', type: 'flat',       emoji: '📅' },
];
...
function DSToggle({ label, checked, onChange, disabled, T, dark }) {
    return (
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.6 : 1, userSelect: 'none' }}>
            <div
                ...
                style={{
                    ...
                    background: checked ? '#7c3aed' : (dark ? 'rgba(148,163,184,0.2)' : '#d1d5db'),
...
function DSSpinner() {
    return (
        <svg width="14" height="14" fill="none" viewBox="0 0 24 24"
            style={{ animation: 'dsSpin 0.7s linear infinite', display: 'inline-block' }}>
...
export default function DeductionSection({ deductions }) {
    ...
    const handleEdit = (ded) => {
        setFormErrors({});
        setData({ name: ded.name, deduction_type: ded.deduction_type, amount_per_unit: ded.amount_per_unit, is_active: !!ded.is_active });
        setEditingId(ded.id);
        setShowForm(true);
    };
    ...
    const handlePreset = (preset) => {
        setData(d => ({ ...d, name: preset.name, deduction_type: preset.type }));
    ...
    return (
        ...
        <style>{`
            ...
            .ds-wrap * { font-family: 'Plus Jakarta Sans', sans-serif; box-sizing: border-box; }
            .ds-row:hover td { background: ${T.rowHover}; }
            .ds-inp:focus { border-color: ${T.borderFocus} !important; box-shadow: 0 0 0 3px ${T.primarySoft} !important; }
            .ds-inp[type=number]::-webkit-outer-spin-button,
            .ds-inp[type=number]::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; }
            .ds-inp[type=number] { -moz-appearance: textfield; }
            @keyframes dsSpin  { to { transform: rotate(360deg); } }
            @keyframes ds-fade { from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)} }
            .ds-animate { animation: ds-fade 0.2s ease; }
        `}</style>

        <div className="ds-wrap" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {deductions.length > 0 ? (
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        ...
                        <tbody>
                            {deductions.map((ded, idx) => {
                                const badge  = typeBadge(ded.deduction_type);
                                const preset = PRESET_DEDUCTIONS.find(p => p.name === ded.name);
                                return (
                                    <tr key={ded.id} className="ds-row"
                                        style={{ borderBottom: idx < deductions.length - 1 ? `1px solid ${T.divider}` : 'none', transition: 'background 0.1s' }}>
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                                                <span style={{ fontWeight: 800, color: T.primary, fontSize: 14 }}>
                                                    {ded.deduction_type === 'percentage'
                                                        ? `${Number(ded.amount_per_unit).toFixed(2)}%`
                                        ...
                                        <td style={{ padding: '12px 14px', textAlign: 'center' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                                                ...
                                                <button onClick={() => setDeleteTarget({ id: ded.id, name: ded.name })}
                ...
                <div style={{
                    ...
                    borderRadius: 16, border: `1.5px dashed ${T.emptyBorder}`,
                    background: T.panelSoft, padding: '36px 24px', gap: 8,
                }}>
                    <div style={{
            ...
            {/* ── Form ── */}
            {showForm && (
                <form onSubmit={handleSubmit} className="ds-animate" style={{
                    borderRadius: 16,
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <button type="button" onClick={handleCancel}
                            style={{ width: 28, height: 28, borderRadius: 8, border: `1px solid ${T.border}`, background: T.panelSoft, color: T.textMute, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>
                            ✕
                        </button>
                    </div>
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                                {[
                                    ...
                                    return (
                                        <label key={opt.value}
                                            onClick={() => !processing && setData('deduction_type', opt.value)}
                                            style={{
                                                flex: 1, padding: '10px 12px', borderRadius: 12,
                                                cursor: processing ? 'not-allowed' : 'pointer',
                                                transition: 'all 0.15s',
                                                border: `1.5px solid ${isSel ? T.primary : T.border}`,
                                                background: isSel ? T.primarySoft : T.panelSoft,
                                            }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                                                <div style={{
                                                    width: 14, height: 14, borderRadius: '50%',
                                                    border: `2px solid ${isSel ? T.primary : T.textMute}`,
                        ...
                        <div>
                            ...
                            <div style={{ position: 'relative' }}>
                                <input
                                    ...
                                    step={data.deduction_type === 'percentage' ? '0.01' : '1'}
                                    min="0"
                                    disabled={processing}
                                    style={{ ...inp(!!formErrors.amount_per_unit), paddingRight: 36 }}
                                />
                            ...
                            <ErrMsg msg={formErrors.amount_per_unit} />
                        </div>
                    </div>
                        ...
                        <div style={{ marginLeft: 'auto', display: 'flex', gap: 10 }}>
                            <button type="submit" disabled={processing}
                                style={{
                                    ...
                                    background: processing ? T.textMute : 'linear-gradient(135deg, #7c3aed, #2563eb)',
                                    color: '#fff', fontSize: 13, fontWeight: 700,
                                    cursor: processing ? 'not-allowed' : 'pointer',
                                    boxShadow: processing ? 'none' : '0 4px 14px rgba(124,58,237,0.35)',
                                    transition: 'all 0.15s',
                                }}
            ...
            {!showForm && (
                <button
                    ...
                    style={{
                        ...
                        border: `1.5px dashed ${T.primary}`, background: T.primarySoft,
